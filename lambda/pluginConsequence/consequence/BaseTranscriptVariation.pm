...
package consequence::BaseTranscriptVariation;
...
use base qw(consequence::VariationFeatureOverlap);
...
sub cds_start {
    ...
    unless (exists $self->{cds_start}) {
        ...
        $self->{cds_start} = $first->isa('consequence::Gap') ? undef : $first->start;
        $self->{cds_end}   = $last->isa('consequence::Gap') ? undef : $last->end ;
    }
...
sub translation_start {
    ...
    unless (exists $self->{translation_start}) {
        ...
        #print Dumper $translation_coords;

        my ($first, $last) = ($translation_coords->[0], $translation_coords->[-1]);
        #print Dumper $first;
        #print Dumper $last;
        $self->{translation_start} = $first->isa('consequence::Gap') ? undef : $first->start;
        $self->{translation_end}   = $last->isa('consequence::Gap') ? undef : $last->end;
    }
    #print Dumper $self->{translation_start};
...
sub translation_coords {
    ...
    #print("\n HIII");
    #print Dumper $self->{_translation_coords};
...
sub _mapper {
    ...
    #print Dumper $tran->{_variation_effect_feature_cache}->{mapper};
    return $mapper;
}

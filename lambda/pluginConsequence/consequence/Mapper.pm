...
package consequence::Mapper;
use strict;
use integer;

#use Bio::EnsEMBL::Utils::Exception qw(throw warning);
use consequence::Pair;
#use Bio::EnsEMBL::Mapper::IndelPair;
use consequence::Unit;
use consequence::Coordinate;
#use Bio::EnsEMBL::Mapper::IndelCoordinate;
use consequence::Gap;
use Data::Dumper;

#use Bio::EnsEMBL::Utils::Exception qw(throw);
use consequence::Interval;
use consequence::Immutable;
...
sub map_coordinates {
  ...
  if ( !defined $hash->{ uc($id) } ) {
    # one big gap!
    my $gap = consequence::Gap->new( $start, $end );
  ...
  foreach my $i (@{$lr}) {
    ...
    #print($start , "\t", $end, "\t", $i);
    push @{$from_intervals}, consequence::Interval->new($start, $end, $i);
  }
  #print Dumper $from_intervals;
  #
  ...
  my $itree = consequence::Immutable->new($from_intervals);
  ...
  foreach my $i (@{$overlap}) {
    ...
    if ( $start < $self_coord->{'start'} ) {
      # gap detected
      my $gap = consequence::Gap->new( $start, $self_coord->{'start'} - 1, $rank );
      push( @result, $gap );
      push(@paired_result, { 'original' => $gap, 'mapped' => $gap });
    ...
    if ( exists $pair->{'indel'} ) {
      ...
      $res = consequence::Coordinate->new( $target_coord->{'id'},
    ...
    push( @result, $res );
    my $res_ori = consequence::Coordinate->new( $self_coord->{'id'},
...
sub add_map_coordinates {
  ...
  my $from = consequence::Mapper::Unit->new( $contig_id, $contig_start,
                                              $contig_end );
  my $to =
    consequence::Mapper::Unit->new( $chr_name, $chr_start, $chr_end );

  my $pair = consequence::Mapper::Pair->new( $from, $to, $contig_ori );

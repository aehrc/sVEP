...
package consequence::BaseVariationFeatureOverlapAllele;
...
use consequence::Constants qw(%OVERLAP_CONSEQUENCES $DEFAULT_OVERLAP_CONSEQUENCE);
use consequence::VariationEffect qw(overlap);
...
sub get_all_OverlapConsequences {
  ...
  unless ($self->{overlap_consequences}) {
    ...
    OC: for my $oc (@{$self->_get_oc_list($pre)}) {
      ...
      #print Dumper $oc;
      #print("here -", $oc->predicate->($self, $feat, $bvfo, $bvf));
...
sub _get_oc_list {
  ...
  #print ($digest);
  #print Dumper $self;
  #print Dumper @SORTED_OVERLAP_CONSEQUENCES;
  ...
  unless(exists($cache->{$digest})) {
    ...
    #print(map {$_->{SO_term}} grep {!$self->_skip_oc($_, $pre)} )
    $cache->{$digest} ||= [
      map {$_->{SO_term}}
      grep {
        $_->{predicate} &&
...
sub _skip_oc {
  ...
  if(my $inc = $oc->{include}) {
    ...
    foreach my $key(keys %$inc) {
      if($inc->{$key} == 0) {
        #print($oc->{SO_term},"\n");
        #if (exists($pre->{$key}) && $pre->{$key} eq $inc->{$key}){
        # print("HI\n");
        #}
        #print("Main \t",$oc->{SO_term},"\n");
        return 1 if exists($pre->{$key}) && $pre->{$key} ne $inc->{$key};
      }
      else {
        #print("In Else \t",$oc->{SO_term},"\n");
        #return 0;
        #print("\nelse - ",$oc->{SO_term},"\n");
...
sub _bvfo_preds {
  ...
  if($bvfo_preds->{within_feature}) {
    ...
    if($preds->{feature_class} eq 'consequence::Transcript') {

      # get coding region coords
      ($tr_coding_start, $tr_coding_end) = ($feat->cdna_coding_start, $feat->cdna_coding_end);

      #print("$tr_coding_start\n$tr_coding_end\n");
    ...
    if($tr_coding_start && $tr_coding_end) {
      ...
      if(!overlap($min_vf, $max_vf, $tr_coding_start, $tr_coding_end)) {
        ...
        if(exists($bvfo_preds->{exon}) && !$bvfo_preds->{exon}) {
          ...
          if(scalar @$cds_coords == 1 && $cds_coords->[0]->isa('consequence::Coordinate')) {

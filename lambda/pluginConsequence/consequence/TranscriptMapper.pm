...
package consequence::TranscriptMapper;
...
sub new {
  ...
  if(!ref($transcript) || !$transcript->isa('consequence::Transcript')) {####this is where we were
...
sub _load_mapper {
  ...
  my $mapper = consequence::Mapper->new( 'cdna', 'genomic');
  ...
  #foreach my $ex (@{$transcript->get_all_Exons}) {}
    my $gen_start = $transcript->cdna_coding_start;
    my $gen_end   = $transcript->cdna_coding_end;

    $cdna_start = $cdna_end + 1;
    $cdna_end   = $cdna_start + $transcript->seq_length - 1;
...
sub genomic2cds {
 ...
 if(!defined($cdna_cstart)) {
   ...
   return consequence::Gap->new($start,$end);
 }
    ...
    # print("start-- ",$start);
     ...
     #} else {
       ...
       my $cds_end   = $end   - $cdna_cstart + 1;
        ...
        # push @out, consequence::Gap->new($start, $cdna_cstart-1);
       ...
       my $end_gap = undef;
        ...
        # $end_gap = consequence::Gap->new($cdna_cend + 1, $end);
...
sub genomic2pep {
 ...
 foreach my $coord (@coords) {
   if($coord->isa('consequence::Gap')) {
     ...
     my $pep_start = int(($coord->start + 2) / 3);
     my $pep_end   = int(($coord->end  + 2) / 3);

...
use consequence::Exception qw(throw);
use consequence::Node;
...
sub query {
  ...
  if (defined $start) {
    ...
    $interval = consequence::Interval->new($start, $end);
  }
...
sub _divide_intervals {
  ...
  my $node = consequence::Node->new($x_center,
								       $s_center,

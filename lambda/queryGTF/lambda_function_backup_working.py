...
def publish_consequences_plugin(slice_data,APIid,batchID):
    ...
    for idx, slice_datum in enumerate(slice_data):
        ...
        if(tot_size < PAYLOAD_SIZE):
            ...
            for topic in topics:
                ...
                kwargs['Message'] = json.dumps({ "snsData" : snsData, "APIid":APIid,"batchID":uniqueBatchID})
...
def annotate_slice(all_coords, all_changes,APIid,batchID):
        overlap_list = overlap_feature(all_coords,all_changes)
        publish_consequences_plugin(overlap_list,APIid,batchID)
...
def lambda_handler(event, context):
    ...
    annotate_slice(coords,changes,APIid,batchID)
